...
income %>%
  ...
  pivot_wider(names_from = year, values_from = n_blocks)
```
...
plot_dkl_bin <- function(city) {
  ...
  df <- income %>%
    ...
    select(group_label, cbsa_fips, cbsa_title, label, dkl_bin, year) %>%
    distinct() %>%
  ...
  df %>%
    ...
    labs(title = "DKL by Income and Year",
         subtitle = paste("for", cbsa),
         x = NULL,
         y = "DKL (bits)",
         fill = "Year") +
