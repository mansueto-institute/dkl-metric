...
pacman::p_load(tidyverse, tidycensus, magrittr, readxl, janitor)
...
get_cbsa_xwalk <- function(xwalk_url, yr) {
  ...
  if (yr == 2009) {
    ...
    cbsa_xwalk <- read_excel(tmp_filepath, sheet = 1, range = cell_rows(lb:ub)) %>%
      clean_names() %>%
      mutate(fips_state_code = str_sub(fips, 1, 2),
             fips_county_code = str_sub(fips, 3)) %>%
      ...
      rename(cbsa_fips = cbsa_code,
             area_type = level_of_cbsa) %>%
      select(county_fips,cbsa_fips,cbsa_title,area_type,county_status)
    } 
  else {
    lb <- 3
    ub <- 1919
