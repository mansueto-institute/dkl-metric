...
from utils.census_api import fetch_all_states, fetch_all_states_tract, fetch_block_group_data
...
def _output_dir_tract(year: int | str, dataset: str) -> Path:
    year_str = str(year).replace("dec", "")
    suffix = f"_{dataset}" if dataset not in ("acs5",) else ""
    d = DATA_DIR / f"acs5_tract_{year_str}_data{suffix}" / "raw"
    d.mkdir(parents=True, exist_ok=True)
    return d
...
def download_year(year: int, var_groups: list[str], geo: str = "blockgroup") -> None:
    """Download ACS 5-year data for a single year.

    Parameters
    ----------
    geo : {'blockgroup', 'tract'}
        Geographic level to download.
    """
    config = YEAR_CONFIGS[year]
    assert config["dataset"] == "acs5", f"Use download_decennial for year={year}"

    if geo == "tract":
        out_dir  = _output_dir_tract(year, "acs5")
        fetch_fn = fetch_all_states_tract
        geo_tag  = "tract"
    else:
        out_dir  = _output_dir(year, "acs5")
        fetch_fn = fetch_all_states
        geo_tag  = "blkgrp"
    ...
    for vg in var_groups:
        ...
        out_path = out_dir / f"{vg}_{geo_tag}_all_states_{year}.parquet"
        if out_path.exists():
            logger.info("Already exists, skipping: %s", out_path.name)
            continue

        logger.info("Downloading ACS5 %d — %s (%s) …", year, vg, geo)
        df = fetch_fn(
            year=year,
            dataset="acs5",
            variables=codes,
...
def download_decennial_2010(var_groups: list[str], geo: str = "blockgroup") -> None:
    """Download 2010 Decennial Census SF1 data (race only)."""
    config = YEAR_CONFIGS["2010dec"]
    out_dir = _output_dir("2010dec", "sf1")

    geo_tag  = "tract" if geo == "tract" else "blkgrp"
    fetch_fn = fetch_all_states_tract if geo == "tract" else fetch_all_states
    out_dir  = _output_dir_tract("2010dec", "sf1") if geo == "tract" else out_dir
    ...
    for vg in var_groups:
        ...
        out_path = out_dir / f"{vg}_{geo_tag}_all_states_2010dec.parquet"
        if out_path.exists():
            logger.info("Already exists, skipping: %s", out_path.name)
            continue

        logger.info("Downloading 2010 SF1 — %s (%s) …", vg, geo)
        df = fetch_fn(
            year=2010,
            dataset="sf1",
            variables=codes,
...
def download_decennial_2000(var_groups: list[str], geo: str = "blockgroup") -> None:
    ...
    if "race" in var_groups:
        sf1_dir = _output_dir_tract("2000dec", "sf1") if geo == "tract" else _output_dir("2000dec", "sf1")
        codes, label_map = DEC_2000_SF1_VAR_GROUPS["race"]
        out_path = sf1_dir / f"race_{geo_tag}_all_states_2000dec.parquet"
        if not out_path.exists():
            logger.info("Downloading 2000 SF1 — race (%s) …", geo)
            df = fetch_fn(
    ...
    for vg in ("income", "educ", "empl"):
        ...
        if vg not in config["available_vars"]:
            continue
        sf3_dir = _output_dir_tract("2000dec", "sf3") if geo == "tract" else _output_dir("2000dec", "sf3")
        codes, label_map = DEC_2000_SF3_VAR_GROUPS[vg]
        out_path = sf3_dir / f"{vg}_{geo_tag}_all_states_2000dec.parquet"
        if not out_path.exists():
            logger.info("Downloading 2000 SF3 — %s (%s) …", vg, geo)
            df = fetch_fn(
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help="Geographic level to download (default: blockgroup).",
    )
    parser.add_argument(
...
if __name__ == "__main__":
    ...
    for yr_str in args.years:
        if yr_str == "2010dec":
            download_decennial_2010(args.vars, geo=args.geo)
        elif yr_str == "2000dec":
            download_decennial_2000(args.vars, geo=args.geo)
        else:
            yr = int(yr_str)
            ...
            if YEAR_CONFIGS[yr]["dataset"] != "acs5":
                ...
                continue
            download_year(yr, args.vars, geo=args.geo)
