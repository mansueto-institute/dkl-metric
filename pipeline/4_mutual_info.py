...
def _dkl_path(year_tag: str, var_group: str) -> Path:
    """Return the expected path for a DKL parquet file.

    year_tag examples: '15', '20', '10'
    """
    # Infer full year from short tag
    full_year = "20" + year_tag if int(year_tag) < 50 else "19" + year_tag
    return (
        DATA_DIR
        / f"acs5_block_{full_year}_data"
        / f"dkl_{var_group}_{year_tag}_blkgrp_all_states.parquet"
    )


def _mi_path(year_tag: str, var_group: str) -> Path:
    full_year = "20" + year_tag if int(year_tag) < 50 else "19" + year_tag
...
def save_mi_file(var_group: str, year_tag: str, *, dkl_path: Path | None = None) -> None:
    ...
    """
    path = dkl_path or _dkl_path(year_tag, var_group)
    out_path = _mi_path(year_tag, var_group)
